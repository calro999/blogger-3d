...
4. HTMLタグ構造（<div class="premium-squishy-article"> 等）を一切崩さず、前置き・解説なしで修正後のHTML本文のみを出力してください。
"""

    github_token = os.environ.get("GITHUB_TOKEN") or os.environ.get("GH_TOKEN")
    if github_token:
        try:
            print("Proofreading and optimizing Blogger article with GitHub Models...")
            ...
            if resp.status_code == 200:
                ...
                if len(res_text.strip()) > 100:
                    print("Successfully proofread and optimized article!")
...
"""
    ...
    system_message = "あなたは楽天ROOMでフォロワー急増中の3Dプリンター・フィラメントや関連パーツ資材専門インフルエンサーです。使い勝手の良いフィラメントや、造形品質が上がる便利な3Dプリンター周辺アクセサリの魅力を、日本語のみで発信してください。"
